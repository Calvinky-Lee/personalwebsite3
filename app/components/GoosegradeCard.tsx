...
const TECH = ["Next.js", "React", "TypeScript", "PostgreSQL", "OpenAI API"];

export default function GoosegradeCard() {
        ...
        <div className="min-w-0">
          <div className="flex items-center min-w-0" style={{ gap: "clamp(6px, 0.7vw, 10px)" }}>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              aria-label="goosegrade.ca"
              className="inline-flex items-center justify-center rounded-full border border-black/[0.1] bg-[#fafafa] hover:bg-[#111] text-[#222] hover:text-white transition-colors leading-none shrink-0 font-semibold"
          ...
          >
            <div className="flex items-start gap-2">
              ...
              <span><span className="text-[#111] font-semibold">7,000+</span> unique users · <span className="text-[#111] font-semibold">16,000+</span> page views</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="shrink-0 text-[#888]">↳</span>
              <span>academic service — extracts weightings for <span className="text-[#111] font-semibold">1,600+</span> courses</span>
            </div>
          </div>
        </div>

        <div className="flex flex-wrap" style={{ gap: "clamp(4px, 0.5vw, 6px)", marginTop: "clamp(10px, 1.1vw, 14px)" }}>
          {TECH.map((t) => (
            <span
              key={t}
              className="text-[#555] border border-black/[0.1] rounded-full bg-[#fafafa] leading-none"
              style={{ fontSize: "clamp(10px, 0.85vw, 12px)", padding: "4px 9px" }}
            >
              {t}
            </span>
          ))}
